...
import { X, Mail, Users, Eye, EyeOff, Edit3, Shield, Lock, Copy, Check, Link as LinkIcon, Zap } from 'lucide-react';
...
export const ShareFileModal: React.FC<ShareFileModalProps> = ({ file, onClose }) => {
  ...
  const handleShare = async () => {
    ...
    setIsSharing(true);
    try {
      const response = await shareService.shareFile(file.id, email.trim(), customPermissions, sharePassword || undefined);
  ...
  const handleUpdateSharePassword = async (share: SharedFile) => {
    ...
    try {
      await shareService.updateSharedFilePermissions(share.id, {
  ...
  const handleClearSharePassword = async (share: SharedFile) => {
